...
export default function ChatWidget() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      text: "Hi, I am CareHub Assistant. I can help with clinic and website questions, doctor info, and booking guidance.",
      doctorCards: null,
    },
  ]);
  ...
  async function sendMessage(rawText) {
    ...
    const nextMessages = [...messages, { role: "user", text, doctorCards: null }];
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", text: assistantText, doctorCards: payload?.doctorCards ?? null },
      ]);
    } catch {
      setMessages((prev) => [
        ...prev,
                ...
                {messages.map((message, index) => (
                  ...
                  >
                    <div className={message.role === "user" ? "max-w-[84%]" : "max-w-[90%]"}>
                      ...
                      message.doctorCards.length > 0 ? null : (
                        <div
                          className={
                            message.role === "user"
                          ...
                          {message.doctorCards.map((doctor, cardIndex) => (
                            ...
                            >
                              ...
                              <div className="mt-2 flex items-start gap-1.5 text-xs text-foreground/75">
                                <CalendarDays size={13} className="mt-0.5 text-primary" />
