...
async function loadPatientDashboard(patientName) {
  const w = scopeForPatient(patientName);
  const now = new Date();
  const weekEnd = new Date(now.getTime() + 7 * DAY_MS);

  const [upcoming, pastVisits, weekCount, doctors, totalDoctors] =
    await Promise.all([
...
export default async function PatientDashboard() {
  ...
  if (!session) redirect("/login?next=/dashboard/patient");
  ...
  if (!user) redirect("/login?next=/dashboard/patient");

  const data = await loadPatientDashboard(user.name);
  ...
  const specialties = [
    {
      ...
      href: "/doctors",
      icon: Brain,
      blurb: `${Math.max(1, Math.floor(data.totalDoctors / 4))}+ in network`,
    },
    {
      name: "Pediatrics",
      href: "/doctors",
      icon: Baby,
      blurb: `${Math.max(1, Math.floor(data.totalDoctors / 3))}+ in network`,
    },
    {
      name: "Dermatology",
      href: "/doctors",
      icon: Sparkles,
      blurb: "Referrals available",
    },
    {
      name: "Mental health",
      href: "/doctors",
      icon: Heart,
      blurb: "Care navigation",
    },
  ];
    ...
    <div className="max-w-7xl mx-auto w-full flex flex-col gap-8">
            ...
            <>
              ...
              <div className="flex-1 min-w-0 flex flex-col">
                ...
                <p className="text-sm text-foreground/55 mt-1">
                  General practice · {formatApptTime(data.nextAppt.scheduledAt)}
                </p>
                ...
                <div className="mt-auto pt-6 flex flex-wrap gap-2">
                  <Link
                    href={`/dashboard/patient/appointments?focus=${data.nextAppt.id}`}
                    className="inline-flex items-center h-10 px-4 rounded-lg bg-primary text-white text-sm font-semibold shadow-sm shadow-primary/20 hover:bg-primary-container transition-colors"
                  >
                    Prepare for visit
                  </Link>
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-xl bg-primary text-white p-6 shadow-sm shadow-primary/25 flex flex-col min-h-[200px]">
            ...
            <div className="mt-3 flex items-baseline gap-2">
              <span className="text-4xl font-black font-manrope tabular-nums">
                92
              </span>
            ...
            <div className="mt-auto pt-4 h-1.5 rounded-full bg-white/20 overflow-hidden">
              <div
                className="h-full w-[92%] rounded-full bg-white/90"
                aria-hidden
              />
            </div>
          </div>
          ...
          <div className="panel p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-lg font-bold font-manrope tabular-nums mt-0.5">
                  120/80{" "}
                  <span className="text-sm font-medium text-foreground/45">mmHg</span>
                </p>
              </div>
            </div>
          </div>
      ...
      <section>
        <PanelHead
          ...
          action={{ href: "/doctors", label: "Browse all" }}
        />
        <p className="text-xs text-foreground/50 mt-1">
      ...
      <section className="panel p-6">
        <PanelHead
          eyebrow="Today"
          title="Available in our network"
          action={{ href: "/doctors", label: "See schedule" }}
        />
        {data.doctors.length === 0 ? (
          ...
          <ul className="mt-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            {data.doctors.map((d, i) => (
              <li
                key={d.id}
                className="flex gap-3 p-3 rounded-lg border border-primary/[0.06] bg-surface-lowest/50"
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-semibold font-manrope text-sm truncate">
                    {d.name}
                  </p>
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    {["9:00", "11:30", "2:15"].map((t) => (
                      <span
                        key={t}
                        className="text-[10px] font-semibold tabular-nums px-2 py-0.5 rounded bg-primary/10 text-primary"
                      >
                        {t}
                      </span>
                    ))}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="panel p-6">
          <PanelHead
            eyebrow="History"
            title="Recent visits"
            action={{ href: "/dashboard/patient/appointments", label: "All visits" }}
          />
          {data.pastVisits.length === 0 ? (
            ...
            <ul className="mt-4 divide-y divide-primary/[0.06]">
              {data.pastVisits.slice(0, 5).map((v) => (
                <li
                  key={v.id}
                  className="flex items-start gap-3 py-3.5 text-sm first:pt-0 last:pb-0"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-semibold truncate">
                      {v.notes?.slice(0, 40) || "Routine visit"}
                    </p>
                    <p className="text-xs text-foreground/50 mt-0.5">
                      {formatApptTime(v.scheduledAt)} ·{" "}
        ...
        <div className="panel p-6">
          <PanelHead
            ...
            action={{ href: "/contact", label: "Pharmacy help" }}
          />
          ...
          <p className="text-xs text-foreground/45 mt-4">
            This area is ready to connect when your clinic enables prescription
            integration.
          </p>
        </div>
      </section>
