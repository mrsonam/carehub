...
import { AppointmentTrendsChart, DoctorBreakdown } from "../../components/admin/AdminCharts";
...
async function loadAdminStats() {
  ...
  const [
    ...
    prisma.appointment.count({
      where: { scheduledAt: { gte: now }, status: { notIn: terminalStatuses } },
    }),
  ...
  for (const row of trendRows) {
    ...
    const idx = Math.round((rowStart - weekStart.getTime()) / (24 * 60 * 60 * 1000));
    if (idx < 0 || idx >= dayBuckets.length) continue;
    if (row.status === "CANCELLED" || row.status === "NO_SHOW") continue;
    if (row.status === "COMPLETED" || row.scheduledAt < now) dayBuckets[idx].completed += 1;
  ...
  return (
    <div className="max-w-7xl mx-auto w-full flex flex-col gap-8">
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Metric icon={UserRound} label="Active patients" value={stats.patientsCount} hint={`${stats.doctorsCount} doctors`} />
      </section>
      ...
      <section className="panel p-6">
        <PanelHead eyebrow="Staff" title="Top performing doctors" action={{ href: "/admin/doctors", label: "View all staff" }} />
        {stats.doctorItems.length === 0 ? (
          ...
          <ul className="mt-4 divide-y divide-primary/[0.06]">
            {stats.doctorItems.map((doc, i) => {
              const pct = Math.round((doc.count / Math.max(1, stats.totalAppointments)) * 100);
              return (
                <li key={doc.name} className="flex items-center gap-4 py-3.5 text-sm first:pt-0 last:pb-0">
                  ...
                  <span className="flex-1 font-semibold font-manrope truncate">{doc.name}</span>
                  <span className="hidden sm:block w-40 text-foreground/55 truncate">General practice</span>
      ...
      <section className="panel p-6">
        <PanelHead
          ...
          action={{ href: "/admin/activity", label: "View all activity" }}
        />
        {stats.recentAppointments.length === 0 ? (
          <p className="mt-6 text-sm text-foreground/50">No activity recorded yet.</p>
        ) : (
          <ul className="mt-4 divide-y divide-primary/[0.06]">
            {stats.recentAppointments.map((appt) => {
              const Icon = appt.status === "COMPLETED" ? CalendarCheck2 : CalendarClock;
              return (
                <li key={appt.id} className="flex items-center gap-4 py-3.5 text-sm first:pt-0 last:pb-0">
                  <span className="w-9 h-9 rounded-lg bg-primary/10 text-primary flex items-center justify-center shrink-0">
                    <Icon size={16} />
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold truncate">
                      {appt.patientName}
                      {appt.doctorName ? (
                        <span className="text-foreground/55 font-normal"> with Dr. {appt.doctorName}</span>
                      ) : null}
                    </p>
                    <p className="text-xs text-foreground/50 mt-0.5">
                      {formatApptTime(appt.scheduledAt)} · {appt.durationMinutes ?? 15} min
                      {appt.patientNotes || appt.notes ? ` · Patient: ${appt.patientNotes ?? appt.notes}` : ""}
                    </p>
                  </div>
