...
/**
 * API routes: current user must be DOCTOR or PATIENT.
 * @returns {{ response: Response } | { user: { id: string; role: string } }}
 */
export async function getProfileEditorOrErrorResponse() {
  const result = await getSessionUserOrErrorResponse();
...
/**
 * @param {"DOCTOR"|"PATIENT"} expectedRole
 * @param {string} loginNext
 */
export async function requireRoleUser(expectedRole, loginNext) {
  const nextQ = encodeURIComponent(loginNext);
  const cookieStore = await cookies();
  const token = cookieStore.get(getSessionCookieName())?.value;
  if (!token) redirect(`/login?next=${nextQ}`);
  const session = await verifySessionToken(token).catch(() => null);
  if (!session) redirect(`/login?next=${nextQ}`);
  if (session.role !== expectedRole) redirect("/dashboard");
  const user = await prisma.user.findFirst({
    where: { id: session.userId, role: expectedRole },
    select: { id: true, name: true, email: true, role: true },
  });
  if (!user) redirect("/dashboard");
  return user;
